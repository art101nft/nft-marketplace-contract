...
contract('Marketplace ERC-1155', function(accounts) {
  ...
  it('disableCollection zeroes and disables collections', async function () {
    ...
    let collectionDetails = await this.mp.collectionState(this.sample1155.address);
    await expect(
      collectionDetails.status
    ).to.equal(false);
    await expect(
      collectionDetails.erc1155
    ).to.equal(false);
    await expect(
  ...
  it('offerTokenForSale requires active contract', async function () {
    ...
    await expectRevert(
      this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(5), {from: accounts[0]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });

  it('offerTokenForSale requires marketplace contract token approval', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 5, "ipfs://mynewhash", {from: accounts[0]});
    await expectRevert(
      this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(5), {from: accounts[0]}),
  ...
  it('offerTokenForSale requires token ownership', async function () {
    ...
    await expectRevert(
      this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(5), {from: accounts[1]}),
  ...
  it('offerTokenForSale puts new offer for token', async function () {
    // update collection
    await this.mp.updateCollection(this.sample1155.address, true, 5, "ipfs://mynewhash", {from: accounts[0]});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    // try offering token as owner, should succeed
    await expectEvent(
      await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(5), {from: accounts[0]}),
      'TokenOffered'
    );
  ...
  it('tokenNoLongerForSale requires active contract', async function () {
    ...
    await expectRevert(
      this.mp.tokenNoLongerForSale(this.sample1155.address, 1, {from: accounts[0]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });
  ...
  it('tokenNoLongerForSale requires token ownership', async function () {
    ...
    // try offerTokenForSale as wrong owner, should fail
    await expectRevert(
      this.mp.tokenNoLongerForSale(this.sample1155.address, 1, {from: accounts[1]}),
      'You must own the token.'
    );
  });
  ...
  it('tokenNoLongerForSale revokes offer for token', async function () {
    ...
    // try revoking offer
    await expectEvent(
      await this.mp.tokenNoLongerForSale(this.sample1155.address, 1, {from: accounts[0]}),
      'TokenNoLongerForSale'
    );
  ...
  it('enterBidForToken requires active contract', async function () {
    ...
    await expectRevert(
      this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });
  ...
  it('enterBidForToken should not require token ownership', async function () {
    ...
    await expectRevert(
      this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[0]}),
  ...
  it('enterBidForToken creates bid for token', async function () {
    ...
    // should be no bid
    await expect(
      (await this.mp.tokenBids(this.sample1155.address, 1)).hasBid
    ).to.equal(false);
    // try revoking offer
    await expectEvent(
      await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)}),
      'TokenBidEntered'
    );
    // bid should be in
  ...
  it('withdrawBidForToken requires active contract', async function () {
    // try enterBidForToken when contract not enabled, should fail
    await expectRevert(
      this.mp.withdrawBidForToken(this.sample1155.address, 1, {from: accounts[1]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });
  ...
  it('withdrawBidForToken cannot allow token ownership', async function () {
    ...
    await expectRevert(
      this.mp.withdrawBidForToken(this.sample1155.address, 1, {from: accounts[0]}),
      'Token owner cannot enter bid to self.'
    );
  });
  ...
  it('withdrawBidForToken should require bid ownership', async function () {
    ...
    await expectRevert(
      this.mp.withdrawBidForToken(this.sample1155.address, 1, {from: accounts[2]}),
  ...
  it('withdrawBidForToken removes bid for token', async function () {
    ...
    // try revoking offer
    await expectEvent(
      await this.mp.withdrawBidForToken(this.sample1155.address, 1, {from: accounts[1]}),
      'TokenBidWithdrawn'
    );
  ...
  it('acceptOfferForToken requires active contract', async function () {
    await expectRevert(
      this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });
  ...
  it('acceptOfferForToken cannot allow token ownership', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 5, "ipfs://mynewhash", {from: accounts[0]});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await expectRevert(
      this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[0]}),
      'Token owner cannot enter bid to self.'
    );
  });
  ...
  it('acceptOfferForToken requires marketplace contract token approval', async function () {
    ...
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await this.sample1155.setApprovalForAll(this.mp.address, false, {from: accounts[0]});
    await expectRevert(
      this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)}),
      'Marketplace not approved to spend token on seller behalf'
    );
  });
  ...
  it('acceptOfferForToken requires enough Ether sent', async function () {
    ...
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await expectRevert(
      this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.9999)}),
  ...
  it('acceptOfferForToken requires seller ownership of token', async function () {
    ...
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await this.sample1155.safeTransferFrom(accounts[0], accounts[5], 1, 1, "0x0");
    await expectRevert(
      this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)}),
  ...
  it('acceptOfferForToken halts sale/active offer', async function () {
    ...
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)});
  ...
  it('acceptOfferForToken gives contract owner their royalty', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 10, "ipfs://mynewhash", {from: accounts[0]});
    ...
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[1]});
    await this.mp.offerTokenForSale(this.sample1155.address, 5, getPrice(1), {from: accounts[1]});
    await this.mp.acceptOfferForToken(this.sample1155.address, 5, {from: accounts[2], value: getPrice(1)});
    await expect(
      await this.sample1155.balanceOf(accounts[1], 5)
  ...
  it('acceptOfferForToken removes existing bid if made by buyer', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 10, "ipfs://mynewhash", {from: accounts[0]});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.8)});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)});
    let bidDetails = await this.mp.tokenBids(this.sample1155.address, 1);
  ...
  it('acceptOfferForToken leaves existing bid if not made by buyer', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 10, "ipfs://mynewhash", {from: accounts[0]});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[2], value: getPrice(.8)});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.offerTokenForSale(this.sample1155.address, 1, getPrice(1), {from: accounts[0]});
    await this.mp.acceptOfferForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(1)});
    let bidDetails = await this.mp.tokenBids(this.sample1155.address, 1);
  ...
  it('acceptBidForToken requires active contract', async function () {
    await expectRevert(
      this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(1), {from: accounts[1]}),
      'Collection must be enabled on this contract by project owner.'
    );
  });
  ...
  it('acceptBidForToken requires token ownership', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 5, "ipfs://mynewhash", {from: accounts[0]});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.5)});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await expectRevert(
      this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(.5), {from: accounts[1]}),
      'You must own the token.'
    );
    await expectRevert(
      this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(.5), {from: accounts[2]}),
      'You must own the token.'
    );
  });
  ...
  it('acceptBidForToken requires marketplace contract token approval', async function () {
    ...
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.5)});
    await expectRevert(
      this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(.5), {from: accounts[0]}),
      'Marketplace not approved to spend token on seller behalf.'
    );
  });
  ...
  it('acceptBidForToken requires bid amount to be greater than seller minimum', async function () {
    ...
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await expectRevert(
      this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(.501), {from: accounts[0]}),
  ...
  it('acceptBidForToken transfers the token to buyer', async function () {
    ...
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.8)});
    await this.sample1155.setApprovalForAll(this.mp.address, true, {from: accounts[0]});
    await this.mp.acceptBidForToken(this.sample1155.address, 1, getPrice(.75), {from: accounts[0]});
    await expect(
      await this.sample1155.balanceOf(accounts[0], 1)
    ).to.be.bignumber.equal('0');
    await expect(
  ...
  it('withdraw sends only allocated funds to msg.sender', async function () {
    await this.mp.updateCollection(this.sample1155.address, true, 10, "ipfs://mynewhash", {from: accounts[0]});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[1], value: getPrice(.5)});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[2], value: getPrice(.525)});
    await this.mp.enterBidForToken(this.sample1155.address, 1, {from: accounts[3], value: getPrice(.55)});
